...
- [x] **TEST-03**: Option B 核心 helper 單元測試 — zero-area fill counter、content stream rewrite correctness(算子序列邊界判定)、form XObject 巢狀偵測、no-op 行為(input 不含 zero-area fill 時)、邊界條件(0 / 1 / 100 / 1742 個 zero-area fill 的密度梯度)
